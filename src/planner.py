...
#                    rot = np.array(rot)
#                    pos = np.array(xyz)
#                    obj_pos_from_base_frame = np.array([gp[j][0],gp[j][1],gp[j][2]])                    
#                    base_pos = calctransform(rot,pos,obj_pos_from_base_frame)
#                    q = quat2quat(np.array(use_qxyzw), np.array([gp[j][4],gp[j][5],gp[j][6],gp[j][7]]))
#                    list(base_pos).extend(list(q))
#                    liks ,_f,_l =  self.calc_next_prob(base_pos, gp[j][3], object_pose, object_category,object_inv,i)
#                    lik = 0.0
#                    for k in range(len(liks)):
#                        lik += trans[i][k] * liks[k]
#                    action_liks_ano[i][j] *= lik
#                    action_liks_ano[i+n][j] *= lik
        ...
        for i in range(n):
            ...
            xyz_t = marge_curve(gp.T , sig.T, xyz_hand_mu, xyz_hand_sig)
            qxyzw_t = gp[:,4:]
            hand = gp[:,3]
...
#            tra_xyz = smooth(trajector[0][:length])
    ...
    def set_motion(self,bow,object_category):
        ...
        cat = self.category[object_category]
        bos = self.bos[cat]
